...
/**
 * Root composable. Reads [MainViewModel.uiState] via StateFlow and renders the
 * status card (live counters + progress bar) and the phase-dependent actions,
 * plus the SAF file picker and the legacy WRITE permission launcher (API <= 28).
 */
...
fun MainScreen(viewModel: MainViewModel = viewModel()) {
    ...
    Scaffold(
        topBar = { TopAppBar(title = { Text("Email Verifier") }) },
    ) { innerPadding ->
        Column(
            modifier = Modifier
                .fillMaxSize()
...
/** One small "value + label" item used inside the status card. */
@Composable
